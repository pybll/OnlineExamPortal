<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="yonetici-y.aspx.cs" Inherits="WebApplication2.yonetici_y" %>
...
<body>
    <form id="form1" runat="server">
        ...
        <div id="cizgi"></div>
        <div id="yy">
            <asp:GridView  ID="GridView1" runat="server" AllowPaging="True" AutoGenerateColumns="False" BackColor="White" BorderColor="#CCCCCC" BorderStyle="None" BorderWidth="1px" CellPadding="4" DataKeyNames="kadi" DataSourceID="yonetici" ForeColor="Black" GridLines="Horizontal" PageSize="5" Font-Bold="False" Font-Italic="False" Font-Names="Calibri" Font-Size="Medium" Font-Strikeout="False" Width="753px">
                <Columns>
                    <asp:CommandField CancelText="İptal" DeleteText="Sil" EditText="Değiştir" InsertText="Ekle" NewText="Yeni" SelectText="Seç" ShowDeleteButton="True" ShowEditButton="True" ShowInsertButton="True" UpdateText="Güncelle" />
                    <asp:BoundField DataField="kadi" HeaderText="kadi" ReadOnly="True" SortExpression="kadi" />
            ...
            <asp:AccessDataSource ID="yonetici" runat="server" ConflictDetection="CompareAllValues" DataFile="~/App_Data/eegitim.accdb" DeleteCommand="DELETE FROM [tblkullanici] WHERE (([kadi] = ?) OR ([kadi] IS NULL AND ? IS NULL)) AND (([sifre] = ?) OR ([sifre] IS NULL AND ? IS NULL)) AND (([ad] = ?) OR ([ad] IS NULL AND ? IS NULL)) AND (([soyad] = ?) OR ([soyad] IS NULL AND ? IS NULL)) AND (([mail] = ?) OR ([mail] IS NULL AND ? IS NULL)) AND (([yetki] = ?) OR ([yetki] IS NULL AND ? IS NULL))" InsertCommand="INSERT INTO [tblkullanici] ([kadi], [sifre], [ad], [soyad], [mail], [yetki]) VALUES (?, ?, ?, ?, ?, ?)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT [kadi], [sifre], [ad], [soyad], [mail], [yetki] FROM [tblkullanici] WHERE ([yetki] = ?) ORDER BY [kadi], [ad], [soyad]" UpdateCommand="UPDATE [tblkullanici] SET [sifre] = ?, [ad] = ?, [soyad] = ?, [mail] = ?, [yetki] = ? WHERE (([kadi] = ?) OR ([kadi] IS NULL AND ? IS NULL)) AND (([sifre] = ?) OR ([sifre] IS NULL AND ? IS NULL)) AND (([ad] = ?) OR ([ad] IS NULL AND ? IS NULL)) AND (([soyad] = ?) OR ([soyad] IS NULL AND ? IS NULL)) AND (([mail] = ?) OR ([mail] IS NULL AND ? IS NULL)) AND (([yetki] = ?) OR ([yetki] IS NULL AND ? IS NULL))">
                <DeleteParameters>
                ...
                <SelectParameters>
                    <asp:Parameter DefaultValue="2" Name="yetki" Type="String" />
                </SelectParameters>
                <UpdateParameters>
                    <asp:Parameter Name="sifre" Type="String" />
                    <asp:Parameter Name="ad" Type="String" />
            ...
            <table>
                <tr>
                    <td class="auto-style1"></td><td>
                       <asp:Label ID="Label3" runat="server" Text="Yeni Yönetici Ekle" Font-Bold="True" Font-Names="Calibri" Font-Size="Large" ForeColor="White"></asp:Label>
                    </td>
                </tr>
                <tr>
