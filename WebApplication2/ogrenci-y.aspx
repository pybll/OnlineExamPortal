<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ogrenci-y.aspx.cs" Inherits="WebApplication2.ogrenci_y" %>
...
<body>
    <form id="form1" runat="server">
        ...
        <div id="cizgi"></div>
        <div id="yy">
            <asp:GridView HorizontalAlign="Center" ID="GridView1" runat="server" AllowPaging="True" AutoGenerateColumns="False" BackColor="White" BorderColor="#CCCCCC" BorderStyle="None" BorderWidth="1px" CellPadding="4" DataKeyNames="kadi" DataSourceID="AccessDataSource1" ForeColor="Black" GridLines="Horizontal" PageSize="5" Font-Bold="False" Font-Italic="False" Font-Names="Calibri" Font-Size="Medium" Font-Strikeout="False" Width="753px">
                <Columns>
            ...
            <asp:AccessDataSource ID="AccessDataSource1" runat="server" ConflictDetection="CompareAllValues" DataFile="~/App_Data/eegitim.accdb" DeleteCommand="DELETE FROM [tblkullanici] WHERE (([kadi] = ?) OR ([kadi] IS NULL AND ? IS NULL)) AND (([sifre] = ?) OR ([sifre] IS NULL AND ? IS NULL)) AND (([ad] = ?) OR ([ad] IS NULL AND ? IS NULL)) AND (([soyad] = ?) OR ([soyad] IS NULL AND ? IS NULL)) AND (([mail] = ?) OR ([mail] IS NULL AND ? IS NULL))" InsertCommand="INSERT INTO [tblkullanici] ([kadi], [sifre], [ad], [soyad], [mail]) VALUES (?, ?, ?, ?, ?)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT [kadi], [sifre], [ad], [soyad], [mail] FROM [tblkullanici] WHERE ([yetki] = ?)" UpdateCommand="UPDATE [tblkullanici] SET [sifre] = ?, [ad] = ?, [soyad] = ?, [mail] = ? WHERE (([kadi] = ?) OR ([kadi] IS NULL AND ? IS NULL)) AND (([sifre] = ?) OR ([sifre] IS NULL AND ? IS NULL)) AND (([ad] = ?) OR ([ad] IS NULL AND ? IS NULL)) AND (([soyad] = ?) OR ([soyad] IS NULL AND ? IS NULL)) AND (([mail] = ?) OR ([mail] IS NULL AND ? IS NULL))">
                <DeleteParameters>
                ...
                <SelectParameters>
                    <asp:Parameter DefaultValue="0" Name="yetki" Type="String" />
                </SelectParameters>
                <UpdateParameters>
                    <asp:Parameter Name="sifre" Type="String" />
                    <asp:Parameter Name="ad" Type="String" />
            ...
            <table>
                <tr>
                    <td class="auto-style1"></td><td>
                       <asp:Label ID="Label3" runat="server" Text="Yeni Öğrenci Ekle" Font-Bold="True" Font-Names="Calibri" Font-Size="Large" ForeColor="White"></asp:Label>
                    </td>
                </tr>
                <tr>
